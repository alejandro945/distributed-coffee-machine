    ...
    stages {
        stage('Clone repository, Copy and build') {
            steps {
                ...
                sh 'sshpass -p "swarch" ssh -o StrictHostKeyChecking=no swarch@10.147.19.95 "cd ci-cd-coffee-machine/src/ && echo 1 > codMaquina.cafe && cd coffeeMach/src/main/resources/ && sed -i \"1s/.*/CoffeMach.Endpoints = default -h 192.168.131.50 -p 12346\" coffeMach.cfg"'
                sh 'sshpass -p "swarch" ssh -o StrictHostKeyChecking=no swarch@10.147.19.107 "cd ci-cd-coffee-machine/src/ && echo 2 > codMaquina.cafe && cd coffeeMach/src/main/resources/ && sed -i \"1s/.*/CoffeMach.Endpoints = default -h 192.168.131.51 -p 12346\" coffeMach.cfg"'
