...
public class ControladorMQ implements Runnable, ServicioAbastecimiento {
	...
	// @Constructor
	public ControladorMQ(ProxyServicePrx proxyServicePrx, VentaServicePrx ventasS, AlarmaServicePrx alarmaS) {
		this.proxyServicePrx = proxyServicePrx;
		this.ventasService = ventasS;
		alarmaServicePrx = alarmaS;
	}
	...
	private void readMachineCode() {
		...
		try {
			...
			code = Integer.parseInt(buffer.readLine());
			entrada.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
